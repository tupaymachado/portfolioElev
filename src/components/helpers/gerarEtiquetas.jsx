...
export function verificaEtiquetasPreco(docData, item, setPrecos) { //verifica se o item precisa ter uma etiqueta de preço impressa
    if (docData.localizacao && item.precoAtual > 0) {
...
export function verificaEtiquetasPromo(docData, item, setPromos, setForaPromos) { //verifica se o item precisa ter uma etiqueta de promoção impressa ou retirada
    if (docData.localizacao && item.promocao === true && !docData.promocao || docData.localizacao && item.promocao === true && item.dataPromocao.getTime() >= docData.dataPromocao.toDate().getTime()) { //se o preço é 0, é pq tiraram de promoção mas não mudaram o status, não sei pq essa viagem
        let obj = {
            ...item,
            unidade: docData.unidade,
            localizacao: docData.localizacao,
    ...
    } else if (docData.localizacao && !docData.promocao && item.promocaoStatus === false || docData.localizacao && item.dataPromocao.getTime() >= docData.dataPromocao.toDate().getTime() && item.promocao == false) { //se o preço é 0, será que eu retiro a etiqueta???
        let obj = {
            ...item,
            unidade: docData.unidade,
            localizacao: docData.localizacao,
